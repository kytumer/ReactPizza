...
const Cart = () => {
    ...
    const { pizzaItems, totalprice } = useSelector(state => state.cartSlice)

    console.log(pizzaItems)

    const finalSum = pizzaItems.reduce((sum, obj) => { return ((obj.price * obj.count) + sum) }, 0)

    const totalAmount = pizzaItems.reduce((sum, obj) => { return (sum + obj.count) }, 0)
        ...
        <div className="container container--cart">
            <div className="cart">
                ...
                <div className={styles.root}>
                    <ul> {
                        pizzaItems.map((element) => <li key={element.id}><CartItem   {...element} /></li>)
                    }</ul>
                </div>
                <div className="cart__bottom">
                    <div className="cart__bottom-details">
                        <span> Всего пицц: <b>{totalAmount} шт.</b> </span>
                        <span> Сумма заказа: <b>{finalSum} ₽</b> </span>
                    </div>
